...
export const f1Api = createApi({
  ...
  endpoints: (builder) => ({
    getDrivers: builder.query<DriverDataType[], void>({
      query: () => "drivers",
      transformResponse: (response: { drivers: DriverDataType[] }) =>
        response.drivers,
    }),
    getTeams: builder.query<TeamDataType[], void>({
      query: () => "teams",
      transformResponse: (response: { teams: TeamDataType[] }) =>
        response.teams,
    }),
    getCircuits: builder.query<CircuitDataType[], void>({
      query: () => "circuits",
      transformResponse: (response: { circuits: CircuitDataType[] }) =>
        response.circuits,
    }),
    getCurrentRaces: builder.query<RaceDataType[], void>({
      query: () => "current",
      transformResponse: (response: { races: RaceDataType[] }) =>
        response.races,
    }),
    getStandings: builder.query<StandingsDataType[], number>({
      query: (year) => `${year}/drivers-championship`,
      transformResponse: (response: {
        drivers_championship: StandingsDataType[];
